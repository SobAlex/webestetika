<form action="{{ route('contact.hero') }}" method="POST"
    ...
    <div class="flex-1">
        ...
        @if (isset($errors) && $errors->has('name'))
            <p id="name_hero{{ $idSuffix }}_error" class="mt-1 text-sm text-red-600">{{ $errors->first('name') }}
            </p>
        @endif
    </div>
    ...
    <div class="flex-1">
        ...
        @if (isset($errors) && $errors->has('phone'))
            <p id="phone_hero{{ $idSuffix }}_error" class="mt-1 text-sm text-red-600">{{ $errors->first('phone') }}
            </p>
        @endif
